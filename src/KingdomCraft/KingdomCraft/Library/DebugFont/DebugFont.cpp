...
bool DebugFont::Init(ID3D11Device* _pDevice, ID3D11DeviceContext* _pDeviceContext, HWND _hWnd)
{
	...
	if (!InitResourceView())
	{
		MessageBox(m_hWnd, "InitResourceViewが失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}

	if (!InitConstantBuffer())
	{
		MessageBox(m_hWnd, "InitConstantBufferが失敗しました", "エラー", MB_ICONSTOP);
		ReleaseResourceView();
		return false;
	}
	...
	if (!InitShader())
	{
		MessageBox(m_hWnd, "InitShaderが失敗しました", "エラー", MB_ICONSTOP);
	...
	if (!InitSamplerState())
	{
		MessageBox(m_hWnd, "InitSamplerStateが失敗しました", "エラー", MB_ICONSTOP);
		ReleaseShader();
		ReleaseConstantBuffer();
		ReleaseResourceView();
		return false;
	...
	if (!InitBlendState())
	{
		MessageBox(m_hWnd, "InitBlendStateが失敗しました", "エラー", MB_ICONSTOP);
		ReleaseSamplerState();
		ReleaseShader();
		ReleaseConstantBuffer();
		ReleaseResourceView();
	...
	{
		MessageBox(m_hWnd, "InitDepthStencilStateが失敗しました", "エラー", MB_ICONSTOP);
		ReleaseBlendState();
		ReleaseSamplerState();
		ReleaseShader();
		ReleaseConstantBuffer();
	...
	OutputDebugString("DebugFontの初期化に成功しました\n");
...
bool DebugFont::InitFont(float _FontHeight, float _FontWidth, D3DXCOLOR _FontColor)
{
	...
	{
		MessageBox(m_hWnd, "頂点データの作成に失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}
...
bool DebugFont::InitResourceView()
{
	...
	if (FAILED(D3DX11CreateShaderResourceViewFromFile(
		...
		NULL)))
	{
		MessageBox(m_hWnd, "テクスチャリーソースビューの生成に失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}

	OutputDebugString("DebugFont フォントテクスチャのResourceViewの生成に成功しました\n");
...
bool DebugFont::InitConstantBuffer()
{
	...
	{
		MessageBox(m_hWnd, "コンスタントバッファの作成に失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}

	OutputDebugString("DebugFont 定数バッファの生成に成功しました\n");
...
bool DebugFont::InitShader()
{
	if (!InitVertexShader())
	{
		MessageBox(m_hWnd, "InitVertexShaderが失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}

	if (!InitPixelShader())
	{
		MessageBox(m_hWnd, "InitPixelShaderが失敗しました", "エラー", MB_ICONSTOP);
		ReleaseVertexShader();
		return false;
	}

	OutputDebugString("DebugFont シェーダーの初期化に成功しました\n");
...
bool DebugFont::InitVertexShader()
{
	...
	if (FAILED(D3DX11CompileFromFile(
		...
		NULL)))
	{
		MessageBox(m_hWnd, "VertexShaderのコンパイルに失敗", "エラー", MB_ICONSTOP);
	...
	if (FAILED(m_pDevice->CreateVertexShader(
		...
		&m_pVertexShader)))
	{
		MessageBox(m_hWnd, "VertexShaderの作成に失敗しました", "エラー", MB_ICONSTOP);
	...
	if (FAILED(m_pDevice->CreateInputLayout(
		...
		&m_pLayout)))
	{
		MessageBox(m_hWnd, "InputLayoutの作成に失敗しました", "エラー", MB_ICONSTOP);
	...
	OutputDebugString("DebugFont バーテックスシェーダーの初期化に成功しました\n");
...
bool DebugFont::InitPixelShader()
{
	...
	if (FAILED(D3DX11CompileFromFile(
		...
		NULL)))
	{
		MessageBox(m_hWnd, "PixelShaderのコンパイルに失敗", "エラー", MB_ICONSTOP);
	...
	OutputDebugString("DebugFont ピクセルシェーダーの初期化に成功しました\n");
...
bool DebugFont::InitSamplerState()
{
	...
	{
		MessageBox(m_hWnd, "サンプラーステートの生成に失敗", "エラー", MB_ICONSTOP);
		return false;
	}

	OutputDebugString("DebugFont サンプラーステートの生成に成功しました\n");
...
bool DebugFont::InitBlendState()
{
	...
	{
		MessageBox(m_hWnd, "ブレンドステートの生成に失敗", "エラー", MB_ICONSTOP);
		return false;
	}

	OutputDebugString("DebugFont ブレンドステートの生成に成功しました\n");
...
bool DebugFont::InitDepthStencilState()
{
	...
	{
		OutputDebugString("DebugFont デプスステンシルステートの生成に失敗\n");
		return false;
	}

	OutputDebugString("DebugFont デプスステンシルステートの生成に成功しました\n");
