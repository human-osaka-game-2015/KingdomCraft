...
bool MouseDevice::Init(LPDIRECTINPUT8 _pDInput8, HWND _hWnd)
{
	...
	if (m_pDInput8 == NULL)
	{
		MessageBox(m_hWnd, "DirectInput8オブジェクトが有効ではありません", "エラー", MB_ICONSTOP);
		return false;
	}

	if (FAILED(m_pDInput8->CreateDevice(GUID_SysMouse, &m_pDInputDevice8, NULL))) 
	{
		MessageBox(m_hWnd, "DirectInput8マウスデバイスの生成に失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}

	if (FAILED(m_pDInputDevice8->SetDataFormat(&c_dfDIMouse))) 
	{
		MessageBox(m_hWnd, "DirectInput8マウスデバイスのデータフォーマットの設定に失敗しました", "エラー", MB_ICONSTOP);
	...
	{
		MessageBox(m_hWnd, "DirectInput8マウスデバイスのバッファ設定に失敗しました", "エラー", MB_ICONSTOP);
		m_pDInputDevice8->Release();
		return false;
	}
	...
	{
		MessageBox(m_hWnd, "DirectInput8マウスデバイスの協調レベルの設定に失敗しました", "エラー", MB_ICONSTOP);
		m_pDInputDevice8->Release();
		return false;
	}
	...
	OutputDebugString("DirectInputのMouseDeviceの初期化に成功した\n");
...
void MouseDevice::Release()
{
	...
	OutputDebugString("DirectInputのMouseDeviceを解放しました\n");
}
