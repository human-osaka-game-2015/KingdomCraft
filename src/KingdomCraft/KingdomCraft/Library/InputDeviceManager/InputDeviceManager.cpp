...
bool InputDeviceManager::Init(HWND _hWnd)
{
	...
	if (m_pDInput8 != NULL)
	{
		MessageBox(m_hWnd, "m_pDInputの中身は空ではありません", "エラー", MB_ICONSTOP);
		return false;
	}

	if (FAILED(DirectInput8Create(GetModuleHandle(NULL), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&m_pDInput8, NULL)))
	{
		MessageBox(m_hWnd, "DirectInput8オブジェクトの生成に失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}

	OutputDebugString("DirectInput8オブジェクトの生成に成功\n");
...
void InputDeviceManager::Release()
{
	if (m_pKeyDevice != NULL)
	{
		...
		OutputDebugString("KeyDeviceクラスを破棄しました\n");
	}
	...
	if (m_pMouseDevice != NULL)
	{
		...
		OutputDebugString("MouseDeviceクラスを破棄しました\n");
	}
...
bool InputDeviceManager::CreateKeyDevice()
{
	if (m_pKeyDevice != NULL)
	{
		MessageBox(m_hWnd, "m_pKeyDeviceの中身は空ではありません", "エラー", MB_ICONSTOP);
		return false;
	}
	...
	{
		MessageBox(m_hWnd, "キーデバイスの初期化に失敗しました", "エラー", MB_ICONSTOP);
		delete m_pKeyDevice;
		m_pKeyDevice = NULL;
		return false;
	}

	OutputDebugString("KeyDeviceクラスの生成に成功\n");
...
bool InputDeviceManager::CreateMouseDevice()
{
	if (m_pMouseDevice != NULL)
	{
		MessageBox(m_hWnd, "m_pMouseDeviceの中身は空ではありません", "エラー", MB_ICONSTOP);
		return false;
	}
	...
	{
		MessageBox(m_hWnd, "マウスデバイス初期化に失敗しました", "エラー", MB_ICONSTOP);
		delete m_pMouseDevice;
		m_pMouseDevice = NULL;
		return false;
	}

	OutputDebugString("MouseDeviceクラスの生成に成功\n");
