...
bool KeyDevice::Init(LPDIRECTINPUT8 _pDInput8, HWND _hWnd)
{
	...
	{
		MessageBox(m_hWnd, "DirectInput8キーデバイスの生成に失敗しました", "エラー", MB_ICONSTOP);
		return false;
	}

	if (FAILED(m_pDInputDevice8->SetDataFormat(&c_dfDIKeyboard)))
	{
		MessageBox(m_hWnd, "DirectInput8キーデバイスのデータフォーマットの設定に失敗しました", "エラー", MB_ICONSTOP);
	...
	{
		MessageBox(m_hWnd, "DirectInput8キーデバイスのバッファ設定に失敗しました", "エラー", MB_ICONSTOP);
		m_pDInputDevice8->Release();
		return false;
	}
	...
	{
		MessageBox(m_hWnd, "DirectInput8キーデバイスの協調レベルの設定に失敗しました", "エラー", MB_ICONSTOP);
		m_pDInputDevice8->Release();
		return false;
	}
	...
	OutputDebugString("DirectInputのKeyDeviceの初期化に成功した\n");
...
void KeyDevice::Release()
{
	...
	OutputDebugString("DirectInputのKeyDeviceを解放しました\n");
}
