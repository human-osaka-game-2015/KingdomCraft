...
bool Vertex2D::InitVertexBuffer(RECT* _pScale, D3DXVECTOR2* _pUV)
{
	...
	{
		MessageBox(m_hWnd, "バーテックスバッファーの生成に失敗しました", "Error", MB_ICONSTOP);
		return false;
	}
...
bool Vertex2D::InitVertexShader()
{
	...
	if (FAILED(D3DX11CompileFromFile(
		...
		NULL)))
	{
		MessageBox(0, "VertexShaderのコンパイルに失敗", "Error", MB_ICONSTOP);
		pErrors->Release();
		return false;
	}
...
bool Vertex2D::InitPixelShader()
{
	...
	if (FAILED(D3DX11CompileFromFile(
		...
		NULL)))
	{
		MessageBox(m_hWnd, "PixelShaderのコンパイルに失敗", "Error", MB_ICONSTOP);
		pErrors->Release();
		return false;
	}
...
bool Vertex2D::LoadTexture(char* _textureFileName)
{
	...
	{
		MessageBox(m_hWnd, "アルファブレンドステート作成に失敗しました", "Error", MB_ICONSTOP);
		return false;
	}
	...
	{
		MessageBox(m_hWnd, "サンプラーステートの生成に失敗しました", "Error", MB_ICONSTOP);
		return false;
	}

	if (FAILED(D3DX11CreateShaderResourceViewFromFile(m_pDevice, _textureFileName, NULL, NULL, &m_pTexture, NULL)))
	{
		MessageBox(m_hWnd, "テクスチャーを読み込めません", "Error", MB_ICONSTOP);
		return false;
	}
	return true;
}
...
bool Vertex2D::InitConstantBuffer()
{
	...
	{
		MessageBox(m_hWnd, "コンスタントバッファーの生成に失敗しました", "Error", MB_ICONSTOP);
		return false;
	}
	return true;
}
